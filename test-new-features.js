...
async function testNewFeatures() {
  ...
  const overallRate =
    totalTests > 0 ? ((totalPassed / totalTests) * 100).toFixed(0) : 0;
  console.log("\n" + "=".repeat(50));
  console.log(
  ...
  if (naTotal > 0) {
    ...
    console.log(
      `  Discovery: ${((naScore.discovery / naTotal) * 100).toFixed(0)}%`,
    );
    console.log(
      `  Precision: ${((naScore.precision / naTotal) * 100).toFixed(0)}%`,
    );
    console.log(
      `  Support: ${((naScore.support / naTotal) * 100).toFixed(0)}%`,
    );
  }
}
