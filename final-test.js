...
const support =
  (featureLines["src/webrtc-integration.js"] || 0) +
  (featureLines["src/tab-groups.js"] || 0);

const total = discovery + precision + support;
if (total > 0) {
  console.log("\n🌟 Natural Asymmetry Distribution:\n");
  console.log(
    `  Discovery: ${((discovery / total) * 100).toFixed(0)}% (target: 30%)`,
  );
  console.log(
    `  Precision: ${((precision / total) * 100).toFixed(0)}% (target: 20%)`,
  );
  console.log(
    `  Support: ${((support / total) * 100).toFixed(0)}% (target: 50%)`,
  );
}
...
Object.entries(integrated).forEach(([feature, isIntegrated]) => {
  console.log(
    `  ${isIntegrated ? "✅" : "❌"} ${feature} integrated in browser.html`,
  );
});
...
const traditionalTotal = Object.values(traditionalLines).reduce(
  (a, b) => a + b,
  0,
);
...
console.log(
  `  Traditional approach: ~${traditionalTotal.toLocaleString()} lines`,
);
