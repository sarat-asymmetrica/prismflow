...
class NaturalAsymmetryOrchestrator {
    ...
    classifyTask(task, context) {
        ...
        if (taskLower.includes('calculate') ||
    ...
    getMetrics() {
        return {
            ...this.metrics,
            localPercentage: (this.metrics.localProcessed / this.metrics.totalTasks * 100).toFixed(1),
            avgCostSaving: (this.metrics.costSaved / this.metrics.totalTasks).toFixed(4),
            avgTimeSaving: (this.metrics.timesSaved / this.metrics.totalTasks).toFixed(0)
        };
    }
