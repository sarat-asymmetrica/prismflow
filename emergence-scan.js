...
const apiPatterns = [
  {
    ...
    pattern: /Notification|requestPermission/g,
    found: false,
  },
  ...
  { name: "Clipboard API", pattern: /navigator\.clipboard/g, found: false },
  {
    ...
    pattern: /showOpenFilePicker|showSaveFilePicker/g,
    found: false,
  },
  ...
  { name: "Payment Request", pattern: /PaymentRequest/g, found: false },
  {
    ...
    pattern: /navigator\.credentials/g,
    found: false,
  },
];
...
sortedPatterns.forEach(([pattern, count]) => {
  console.log(
    `  ${pattern}: ${count} files (${((count / files.length) * 100).toFixed(0)}%)`,
  );
});
...
const uxFeatures = {
  "Dark mode toggle": !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("dark-mode"),
  ),
  "Keyboard shortcuts help": !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("shortcuts-help"),
  ),
  "Tab groups": !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("tab-group"),
  ),
  "Reading mode": !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("reading-mode"),
  ),
  Translation: !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("translate"),
  ),
  "Print preview": !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("print-preview"),
  ),
  "Find in page": !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("find-in-page"),
  ),
  "Password manager": !files.some((f) =>
    fs.readFileSync(path.join(srcDir, f), "utf8").includes("password"),
  ),
};
...
const fileTypes = {
  discovery: files.filter((f) => f.includes("test") || f.includes("demo"))
    .length,
  precision: files.filter(
    (f) =>
      f.includes("util") || f.includes("helper") || f.includes("optimizer"),
  ).length,
  support: files.filter(
    (f) => f.includes("ui") || f.includes("component") || f.includes("panel"),
  ).length,
};
...
if (total > 0) {
  ...
  console.log(
    `  Discovery: ${((fileTypes.discovery / total) * 100).toFixed(0)}% (target: 30%)`,
  );
  console.log(
    `  Precision: ${((fileTypes.precision / total) * 100).toFixed(0)}% (target: 20%)`,
  );
  console.log(
    `  Support: ${((fileTypes.support / total) * 100).toFixed(0)}% (target: 50%)`,
  );
}
...
if (patterns.caching > 5) {
  recommendations.push(
    "Caching pattern is emerging - consider unified cache system",
  );
}
...
if (largeFiles.length > 0) {
  recommendations.push(
    `Split ${largeFiles[0].file} for better maintainability`,
  );
}
