...
const browserHTML = fs.readFileSync(
  path.join(__dirname, "src/browser.html"),
  "utf8",
);
const mainJS = fs.readFileSync(
  path.join(__dirname, "src/main-complete.js"),
  "utf8",
);
  ...
  for (const item of items) {
    ...
    if (!elementExists) {
      results.missing.push(
        `${category}/${item.id || item.class || item.category}`,
      );
      console.log(
        `  ❌ ${item.id || item.class || item.category} - NOT IN HTML`,
      );
      continue;
    }
    ...
    if (item.handler) {
      ...
      const eventPattern = new RegExp(
        `(addEventListener|onclick|on\\w+).*${item.id || item.class}`,
        "i",
      );
      const hasEventListener =
        eventPattern.test(browserHTML) || eventPattern.test(mainJS);
      ...
      if (handlerInHTML || handlerInMain) {
        if (hasEventListener) {
          ...
          console.log(
            `  ⚠️  ${item.id || item.class} - HANDLER EXISTS BUT NO LISTENER`,
          );
        }
      } else {
        results.partial.push(`${category}/${item.id || item.class}`);
        console.log(
          `  ⚠️  ${item.id || item.class} - ELEMENT EXISTS BUT NO HANDLER`,
        );
      }
    } else {
...
console.log(
  `  Total: ${(parseFloat(htmlSize) + parseFloat(jsSize)).toFixed(1)} KB`,
);
