    ...
    const update = (currentTime: number) => {
      ...
      this.animations.forEach((animation, id) => {
        ...
        if (progress < 1) {
          ...
          this.applyValue(animation.element, animation.property, value);
        } else {
          // Animation complete
          this.applyValue(
            animation.element,
...
class ErrorHandler {
  ...
  private normalize(error: unknown): ApplicationError {
    ...
    if (error instanceof Error) {
      ...
      // Default error
      return new ApplicationError(error.message, ErrorCode.UNKNOWN_ERROR, 500, {
...
// Usage example
const safeFunction = errorHandler.wrapAsync(async (data: any) => {
