...
export const StatusUpdateModal = ({
  ...
  const currentStageConfig = STAGES.find((s) => s.id === delivery.stage);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        `Delivery status updated to ${STAGES.find((s) => s.id === newStage)?.name}`,
      );
      handleClose();
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-4">
          ...
          <div>
            ...
            <Select
              value={newStage}
              onValueChange={(value) => setNewStage(value as Stage)}
            >
