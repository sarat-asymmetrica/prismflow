...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const removeCustomer = (customer: string) => {
    ...
    onFiltersChange({
      ...filters,
      customer: current.filter((c: unknown) => c !== customer),
    });
  };
  ...
  const removeProduct = (product: string) => {
    ...
    onFiltersChange({
      ...filters,
      product: current.filter((p: unknown) => p !== product),
    });
  };
  ...
  const removeSalesperson = (person: string) => {
    ...
    onFiltersChange({
      ...filters,
      salesperson: current.filter((s: unknown) => s !== person),
    });
  };
    ...
    <Card className="p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          ...
          {filters.customer && filters.customer.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {filters.customer.map((customer: unknown) => (
                <Badge
                  key={customer}
                  variant="secondary"
        ...
        <div>
          ...
          {filters.product && filters.product.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {filters.product.map((product: unknown) => (
                <Badge
                  key={product}
                  variant="secondary"
                  className="gap-1 cursor-pointer"
        ...
        <div>
          ...
          {filters.salesperson && filters.salesperson.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {filters.salesperson.map((person: unknown) => (
                <Badge
                  key={person}
                  variant="secondary"
                  className="gap-1 cursor-pointer"
